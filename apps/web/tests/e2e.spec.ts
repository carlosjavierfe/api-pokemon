...
async function mockApi(route: Route, finished = false) {
...
test("inicia una partida standard y resuelve una ronda con dos opciones", async ({
  ...
  await expect(page.getByRole("heading", { name: "Es bulbasaur" })).toBeVisible();
  await expect(
...
test("mantiene accesibles el inicio y el resultado sin overflow horizontal", async ({
  ...
  await expect(page.locator('input[placeholder="Escribe tu nombre, ej. Ash"]')).toBeVisible();
  await expect(page.locator("#answer")).toHaveCount(0);
  await page.getByRole("button", { name: "Activar audio" }).click();
  await expect(page.getByRole("button", { name: "Desactivar audio" })).toHaveAttribute("aria-pressed", "true");
  await page.getByLabel("Nombre de entrenador").fill("Ash");
  await page.getByRole("button", { name: "Comenzar" }).click();
  await expect(page.getByText("Ronda 1 / 10")).toBeVisible();
  ...
  await page.locator(".choice-list button").first().click();
  const resultHeading = page.getByRole("heading", { name: "Es bulbasaur" });
...
test("muestra el resumen al terminar standard", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Buen trabajo, Ash" })).toBeVisible();
  await expect(page.locator(".final-summary dd").nth(0)).toHaveText("780");
  await expect(page.locator(".final-summary dd").nth(1)).toHaveText("10");
  await expect(page.locator(".final-summary dd").nth(2)).toHaveText("3");
});
